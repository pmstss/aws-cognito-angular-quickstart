...
createCognitoResources() {
    ...
    $aws_cmd cognito-identity create-identity-pool --identity-pool-name $IDENTITY_POOL_NAME --allow-unauthenticated-identities --region $REGION| grep IdentityPoolId | awk '{print $2}' | xargs |sed -e 's/^"//'  -e 's/"$//' -e 's/,$//' > $TMP_PATH/poolId
    IDENTITY_POOL_ID=$(cat $TMP_PATH/poolId)
    echo "Created an identity pool with id of " $IDENTITY_POOL_ID

    # Create an IAM role for unauthenticated users
    cat unauthrole-trust-policy.json | sed 's/IDENTITY_POOL/'$IDENTITY_POOL_ID'/' > $TMP_PATH/unauthrole-trust-policy.json
    $aws_cmd iam create-role --role-name $ROLE_NAME_PREFIX-unauthenticated-role --assume-role-policy-document file://$TMP_PATH/unauthrole-trust-policy.json > $TMP_PATH/iamUnauthRole
    if [ $? -eq 0 ]
    then
        echo "IAM unauthenticated role successfully created"
    else
        echo "Using the existing role ..."
        $aws_cmd iam get-role --role-name $ROLE_NAME_PREFIX-unauthenticated-role  > $TMP_PATH/iamUnauthRole
        $aws_cmd iam update-assume-role-policy --role-name $ROLE_NAME_PREFIX-unauthenticated-role --policy-document file://$TMP_PATH/unauthrole-trust-policy.json
    fi
    ...
    cat authrole-trust-policy.json | sed 's/IDENTITY_POOL/'$IDENTITY_POOL_ID'/' > $TMP_PATH/authrole-trust-policy.json
    $aws_cmd iam create-role --role-name $ROLE_NAME_PREFIX-authenticated-role --assume-role-policy-document file://$TMP_PATH/authrole-trust-policy.json > $TMP_PATH/iamAuthRole
    if [ $? -eq 0 ]
    then
        echo "IAM authenticated role successfully created"
    else
        echo "Using the existing role ..."
        $aws_cmd iam get-role --role-name $ROLE_NAME_PREFIX-authenticated-role  > $TMP_PATH/iamAuthRole
        $aws_cmd iam update-assume-role-policy --role-name $ROLE_NAME_PREFIX-authenticated-role --policy-document file://$TMP_PATH/authrole-trust-policy.json
    fi
    cat authrole.json | sed 's~DDB_TABLE_ARN~'$DDB_TABLE_ARN'~' > $TMP_PATH/authrole.json
    $aws_cmd iam put-role-policy --role-name $ROLE_NAME_PREFIX-authenticated-role --policy-name CognitoPolicy --policy-document file://$TMP_PATH/authrole.json

    # Create the user pool
    $aws_cmd cognito-idp create-user-pool --pool-name $POOL_NAME --auto-verified-attributes email --policies file://user-pool-policy.json --region $REGION > $TMP_PATH/$POOL_NAME-create-user-pool
    USER_POOL_ID=$(grep -E '"Id":' $TMP_PATH/$POOL_NAME-create-user-pool | awk -F'"' '{print $4}')
    echo "Created user pool with an id of " $USER_POOL_ID

    # Create the user pool client
    $aws_cmd cognito-idp create-user-pool-client --user-pool-id $USER_POOL_ID --no-generate-secret --client-name webapp --region $REGION > $TMP_PATH/$POOL_NAME-create-user-pool-client
    USER_POOL_CLIENT_ID=$(grep -E '"ClientId":' $TMP_PATH/$POOL_NAME-create-user-pool-client | awk -F'"' '{print $4}')
    ...
    $aws_cmd cognito-identity update-identity-pool --allow-unauthenticated-identities --identity-pool-id $IDENTITY_POOL_ID --identity-pool-name $IDENTITY_POOL_NAME \
        ...
        > $TMP_PATH/$IDENTITY_POOL_ID-add-user-pool

    # Update cognito identity with the roles
    UNAUTH_ROLE_ARN=$(perl -nle 'print $& if m{"Arn":\s*"\K([^"]*)}' $TMP_PATH/iamUnauthRole | awk -F'"' '{print $1}')
    AUTH_ROLE_ARN=$(perl -nle 'print $& if m{"Arn":\s*"\K([^"]*)}' $TMP_PATH/iamAuthRole | awk -F'"' '{print $1}')
...
createDDBTable() {
    ...
    then
        ...
        $aws_cmd dynamodb describe-table --table-name $TABLE_NAME > $TMP_PATH/dynamoTable
    fi

    DDB_TABLE_ARN=$(perl -nle 'print $& if m{"TableArn":\s*"\K([^"]*)}' $TMP_PATH/dynamoTable | awk -F'"' '{print $1}')
}
...
createS3Bucket() {
    # Create the bucket
    $aws_cmd s3 mb s3://$BUCKET_NAME/ --region $REGION 2>$TMP_PATH/s3-mb-status
    status=$?
    ...
    then
        ...
        if grep "BucketAlreadyOwnedByYou" $TMP_PATH/s3-mb-status > /dev/null
        then
...
uploadS3Bucket() {
    ...
    cat s3-bucket-policy.json | sed 's/BUCKET_NAME/'$BUCKET_NAME'/' > $TMP_PATH/s3-bucket-policy.json
    $aws_cmd s3api put-bucket-policy --bucket $BUCKET_NAME --policy file://$TMP_PATH/s3-bucket-policy.json  --region $REGION
